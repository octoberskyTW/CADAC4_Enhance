...
//Module-variable locations are assigned to flat6[210-249]
// 
...
void Flat6::def_newton()
{
    ...
    flat6[0].init("time",0,"Simulation time - sec","newton","exec","scrn,plot,com");
    flat6[1].init("halt","int",0,"=1: vehicle object is halted - ND","newton","exec","");
    flat6[210].init("VBEBD",0,0,0," Derivative of plane velocity - m/s^2","newton","state","");
    flat6[213].init("VBEB",0,0,0,"Plane velocity in body axes - m/s","newton","state","plot");
    flat6[216].init("SBELD",0,0,0,"Derivative of plane position - m/s","newton","state","");
    flat6[219].init("SBEL",0,0,0,"Plane pos. wrt point E in local level axes - m","newton","state","plot,com");
    ...
    flat6[223].init("SBELM",0,0,0,"Saved aircraft position SBEL - m","newton","save","");
    flat6[224].init("groundrange",0,"ground range - m","newton","diag","");
    flat6[230].init("FSPB",0,0,0,"Specific force in body axes - m/s^2","newton","out","");
    flat6[233].init("VBEL",0,0,0,"Plane velocity in local level axes - m/s","newton","out","com,scrn,plot");
    flat6[236].init("dvbe",0,"Plane speed - m/s","newton","in/out","plot");
    ...
    flat6[243].init("alx",0,"Lateral (horizontal) specific force component - g's","newton","diag","plot");
    flat6[244].init("anx",0,"Normal specific force component - g's","newton","diag","scrn,plot");
    flat6[245].init("ayx",0,"Side specific force component - g's","newton","diag","plot");
...
//Member function of class 'Flat6'
//Initializing plane position and velocities
// 
//020513 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////
...
void Flat6::init_newton()
{
	...
	//creating displacement vector in local level coord
	SBEL.build_vec3(sbel1,sbel2,sbel3);

	//initialize hbe
	hbe=-SBEL.get_loc(2,0);
	//initializing aircraft postion for ground track calculations
	Matrix SBELM=SBEL;
...
void Flat6::newton(double sim_time,double int_step)
{
	...
	Matrix SBELM=flat6[223].vec();
	double groundrange=flat6[224].real();
	//input from other modules
	...
	//diagnostics: lateral (horizontal) acceleration achieved in g's
	Matrix TVL=mat2tr(psivl,thtvl);
	Matrix TVB=TVL*~TBL;
	Matrix FSPV=TVB*FSPB;
	alx=FSPV[1]/grav;
	...
	//diagnostic: groundrange
	Matrix DELSBEL=SBEL-SBELM;
	//setting vertical component to zero
	DELSBEL[2]=0;
	//ground distance increment
	double delgrd=DELSBEL.absolute();
	groundrange+=delgrd;
	//saving current position for next cycle
	SBELM=SBEL;
	//-------------------------------------------------------------------------
	//loading module-variables
	//state variables
