...
void Flat6::def_kinematics()
{
	//Definition and initialization of module-variables
     ...
     flat6[137].init("psiblx",0,"Yawing angle of vehicle - deg","kinematics","in/di","scrn,plot");
     flat6[138].init("thtblx",0,"Pitching angle of vehicle - deg","kinematics","in/di","scrn,plot");
     flat6[139].init("phiblx",0,"Rolling angle of vehicle - deg","kinematics","in/di","scrn,plot");
     ...
     flat6[146].init("erq",0,"Error of quaternion orthogonality","kinematics","diag","plot");
...
//aimc05_3	double eps=2.*acos(q0);
//aimc05_3	double seps=sin(eps/2);
//aimc05_3	double n1=q1/seps;
//aimc05_3	double n2=q2/seps;
//aimc05_3	double n3=q3/seps;
	//-------------------------------------------------------------------------
...
void Flat6::kinematics(double int_step)
{
	...
	Matrix VBAL=flat6[76].vec();
	Matrix WBEB=flat6[164].vec();
	double trcode=plane[180].real();
	double tralppx=plane[186].real();
	double tralpnx=plane[187].real();
	double trbetx=plane[188].real();
	//state variables
	...
	if(vbab2==0 && vbab3==0)
		phip=0;
	//note: if vbab2 is <EPS the value if phip is forced to be 0 or PI
	...
	if(alphax>tralppx) trcode=5;
	if(alphax<tralpnx) trcode=6;
	if(fabs(betax)>trbetx) trcode=7;
