...
//
//Contains the global functions for the Rotor6 simulation.
//
...
//					&num_rotor, number of rotor rotors					
//
//010330 Created by Peter H Zipfel
...
void number_objects(fstream &input,int &num_vehicles,int &num_rotor)
{
	...
	//searching for # of rotor objects
	...
	do{
		input>>read;
		input.getline(line_clear,CHARL,'\n');
		if (!strcmp(read,"ROTOR")) num_rotor++;

	}while((num_rotor)<num_vehicles);
...
Cadac *set_obj_type(fstream &input,Module *module_list,int num_modules)				   
{
	...
	if (!strcmp(temp,"ROTOR"))
	{
		//the pointer 'obj' is allocated the type 'Rotor' 
		obj=new Rotor(module_list,num_modules);
...
//011022 Changed treatment of plot_file_list;'num_vehicles'replaced by'num_rotor'PZi
...
//				if(file_istream_list[i].gcount()) fmerge<<line;
	...
	fmerge.close();
	for(i=0;i<num_rotor;i++) file_istream_list[i].close();
...
//to be appended, limiting rotor vehicles to 9 each (e.g.: SBEG1_t1)
//Accomodates 3x1 vectors
//
//010207 Created by Peter Zipfel
//011129 Adapted to Rotor6 simulation, PZi
///////////////////////////////////////////////////////////////////////////////
				 ...
				 int nrotor)
{
	...
	string id;
	int loc;
	int i_rotor=0; //first rotor packet index in vehicle_list
	int ncom_rotor6; //number of module variables in 'com_rotor6'
	Variable *data_rotor; //Variable array stored in 'Packet data' of type Rotor
	int i(0),j(0);

	ftraj<<"1"<<title<<" "<< __DATE__ <<" "<< __TIME__ <<"\n";

	//find first rotor packet index in 'combus'
	...
	//number of data elements in 'com_rotor'
	ncom_rotor6=combus[i_rotor].get_ndata();

	//picking the first data packet for label extraction
	data_rotor=combus[i_rotor].get_data();

	//determining the total number of vector variables in com_rotor6
	for(j=0;j<ncom_rotor6;j++)
	{
		buff1=data_rotor[j].get_name();
		if(isupper(buff1[0])) p++;
	}
	//number of variables (with vectors componentalized and only one 'time' label))
	int nvariables=ncom_rotor6*nrotor+2*p*nrotor-(nrotor-1);
	...
	for(i=0;i<num_vehicles;i++)
	{
		//determining 'Rotor' object
		id=combus[i].get_id();
		loc=id.find("m");
		if(!loc)
		//'Rotor' object
		{
			for(int k=0;k<ncom_rotor6;k++)
			{
				buff1=data_rotor[k].get_name();
...
void traj_data(ofstream &ftraj,Packet *combus,int num_vehicles,bool merge)
{
	...
	{
		//determining wether 'Rotor' object
		id=combus[i].get_id();
		loc=id.find("m");
		if(!loc)
		//'Rotor' object
...

{
	...
	int p=0;
	string id;
	int nrotor=0; //number of rotor packets 
	int loc;
	int i_rotor=0; //first rotor packet index in vehicle_list
	int ncom_rotor6; //number of module variables in 'com_rotor6'
	Variable *data_rotor; //Variable array stored in 'Packet data' of type Rotor
	...
	//determining the number of rotor packets
	for(i=0;i<num_vehicles;i++)
	{
		id=combus[i].get_id();
		loc=id.find("m");
	...
	//'Rotor' object labels (exclude 'time')
	for(k=1;k<ncom_rotor6;k++)
	{
		buff1=data_rotor[k].get_name();
	...
	//'Rotor' data output (exclude 'time')
	for(i=0;i<num_vehicles;i++)
	{
		//determining 'Rotor'object
		id=combus[i].get_id();
...
void document_input(Document *doc_rotor6)
{
	...
	input.setf(ios::left);
	do{
		fcopy>>buffo;;//buffo gets ROTOR, ENDTIME STOP and //comments

		//finding ROTOR object
		if(!strcmp(buffo,"ROTOR")){
			input<<'\t'<<buffo;		//writes ROTOR line
			fcopy.getline(line_clear,CHARL,'\n');
			input<<line_clear<<'\n';

			//inside ROTOR loop until 'END' is reached
			do{
				fcopy>>buffn;
				int dum=1;
				...
				//inserting 'END' with only one tab
					...
					def_found=false; 
					for(int k=0;k<(NROTOR);k++){
						if(!strcmp(doc_rotor6[k].get_name(),"end_array"))break;
						if(!strcmp(doc_rotor6[k].get_name(),buffn)){
							fcopy.getline(line_clear,CHARL,'\n'); 							
							input<<"    //";
							if(!strcmp(doc_rotor6[k].get_type(),"int"))input<<"'int' ";
							input<<doc_rotor6[k].get_def();
							input<<"  module ";
							input<<doc_rotor6[k].get_mod();
							input<<'\n';
							*buff_stoch=NULL;
							def_found=true;
						}
