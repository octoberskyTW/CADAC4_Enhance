...
void number_objects(fstream &input,int &num_vehicles,int &num_cruise)
{
	...
	//searching for # of cruise objects
...
Cadac *set_obj_type(fstream &input,Module *module_list,int num_modules)				   
{
	...
	if(!strcmp(temp,"CRUISE3"))
	{
		...
		try {obj=new Cruise(module_list,num_modules);}		
...
//to be appended, limiting cruise vehicles to 14 each (e.g.: SBEG1_t1)
				 ...
				 int ncruise)
{
	...
	//number of data elements in 'com_cruise'
	...
	//determining the total number of vector variables in com_cruise3 
	...
	int nvariables=ncom_cruise3*ncruise+2*(p*ncruise)-(ncruise-1);
	...
	for(i=0;i<num_vehicles;i++)
	{
		//determining whether 'Cruise' object
		id=combus[i].get_id();
...

{
	...
	//determining the number of cruise packets
	for(i=0;i<num_vehicles;i++)
	{
		id=combus[i].get_id();
		loc=id.find("c");
