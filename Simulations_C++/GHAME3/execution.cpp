...
//060927 Made compatible with MS C++8, PZi
//100505 Modified for GHAME3, PZi
//130723 Adapted to MS Visual C++ V10, also compatible with V12 (2013) PZi
///////////////////////////////////////////////////////////////////////////////
...
void number_objects(fstream &input,int &num_vehicles,int &num_cruise);
...
void execute(Vehicle &vehicle_list,Module *module_list,double sim_time,
			 ...
			 int num_cruise,ofstream &ftraj,char *title,bool traj_merge);
...
void document_input(Document *doc_cruise3);
...
int main(void) 
{
	...
	number_objects(input,num_vehicles,num_cruise);
	...
	for(int i=0;i<num_vehicles;i++)
	{
		...
		//composing documentation of 'round3','cruise'  module-variables
		//but do it only once for CRUISE3 objects
		...
		combus[i]=vehicle_list[i]->loading_packet_init(num_cruise);
	...
	if(strstr(options,"y_traj")&&one_traj_banner)
	{
		...
		traj_banner(ftraj,combus,title,num_vehicles,num_cruise);
	...
	execute(vehicle_list,module_list,sim_time,
			 ...
			 plot_ostream_list,combus,status,num_cruise,ftraj,title,
			 traj_merge);
	...
	while (sim_time<=(end_time+int_step))
	{
		...
		for (int i=0;i<num_vehicles;i++)
		{
			...
			if(health==1)
			{
				...
				combus[i]=vehicle_list[i]->loading_packet(num_cruise);
