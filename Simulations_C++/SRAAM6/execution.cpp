...
void execute(Vehicle &vehicle_list,Module *module_list,double sim_time,
			 ...
			 int num_missile,int num_target,ofstream &ftraj,char *title,bool traj_merge);
...
void document_input(Document *doc_missile6,Document *doc_target3);
...
int main() 
{
	...
	int num_missile; //number of missile objects
	int num_target; //number of target objects
	...
	{
		//Loading pointers of the vehicle-object classes ('Missile','Target') into 
		...
		//Furthermore, it passes 'module_list', 'num_modules' and 'num_target'
		// to the 'Missile' and 'Target' constructors
		vehicle_type=set_obj_type(input,module_list,num_modules,num_target);
		...
		//composing documentation of 'flat6', 'missile' and 'target' module-variables
		//but do it only once for MISSILE6 and TARGET3 objects
		if(strstr(options,"y_doc"))
		{
			if(!strcmp(vehicle_name,"MISSILE6")&&document_missile)
			{
				...
				doc_missile6=new Document[size];
				if(doc_missile6==0){cerr<<"*** Error: doc_missile6[] allocation failed *** \n";system("pause");exit(1);}
			...
			if(!strcmp(vehicle_name,"TARGET3")&&document_target)
			{
				...
				int size=NFLAT3+NTARGET;
				doc_target3=new Document[size];
				if(doc_target3==0){cerr<<"*** Error: doc_target3[] allocation failed *** \n";system("pause");exit(1);}
		...
		combus[i]=vehicle_list[i]->loading_packet_init(num_missile,num_target);
	...
	if(strstr(options,"y_traj")&&one_traj_banner)
	{
		...
		traj_banner(ftraj,combus,title,num_vehicles,num_missile,num_target);
		...
		if(strstr(options,"y_doc")){
			...
			document_input(doc_missile6,doc_target3);
	...
	execute(vehicle_list,module_list,sim_time,
			 ...
			 plot_ostream_list,combus,status,num_missile,num_target,ftraj,title,
			 traj_merge);
...
//								and their respective type (missile missile or targets)
			...
			if(health==1)
			{
				...
				for(int j=0;j<num_modules;j++)
				{
					if(module_list[j].name=="environment")
						...
						vehicle_list[i]->seeker(combus,num_vehicles,int_step);
				...
				combus[i]=vehicle_list[i]->loading_packet(num_missile,num_target);
