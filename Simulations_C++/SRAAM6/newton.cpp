...
void Flat6::newton(double sim_time,double int_step)
{
	...
	double trackd=flat6[223].real();
	double track=flat6[224].real();
	//-----------------------------------------------------------------------------
	...
	//integrating ground velocity to obtain ground track distance
	Matrix VGROUND=VBEL;
	VGROUND[2]=0;
	double vground=VGROUND.absolute();
	double trackd_new=vground;
	track=integrate(trackd_new,trackd,track,int_step);
	trackd=trackd_new;
	...
	flat6[219].gets_vec(SBEL);
	flat6[223].gets(trackd);
	flat6[224].gets(track);
	//saving values
