...
//
// (1)Aerodynamic tables are located in file 'sraam6_aero_deck.asc'
//      Ref length refl = 0.1524 m
//		Ref area   refa = 0.01824 m^2
...
void Missile::init_aerodynamics()
{
	...
	//stored in trcond(1772). If mstop(1750)=1 the simulation will stop.
	...
	//		 1     trcvel     c1    minimum closing velocity - ft/s
	//		 2     trmach     g2    minimum mach number
	//		 3     trdynm     g2    minimum dynamic pressure - lbf/ft^2
	//		 4     trload     a1    minimum load factor - g's
	//		 5     tralp      g3    maximum total incidence angle - rad
	//		 6     trtht      s1    maximum pitch gimbal angle - rad
	//		 7     trthtd     s1    maximum pitch gimbal rate - rad/s
	//		 8     trphid     s1    maximum roll gimbal rate  - rad/s
	//		 9     trate      s1    maximum tracking rate - rad/s
	...
	missile[182].gets(trcvel);
	missile[183].gets(trmach);
	missile[184].gets(trdynm);
	missile[185].gets(trload);
	missile[186].gets(tralp);
	missile[187].gets(trtht);
	missile[189].gets(trthtd);
	missile[190].gets(trphid);
	missile[191].gets(trate);
	missile[180].gets(trcode);
	missile[181].gets(tmcode);
}
///////////////////////////////////////////////////////////////////////////////
//Aerodynamic module
//Member function of class 'Missile'
...
void Missile::aerodynamics()
{
	...
	double cndq=aerotable.look_up("cndq_vs_mach_alpha",vmach,alppx);
	...
	cllap=aerotable.look_up("cllap_vs_mach_alpha",vmach,alppx);
	cllp=aerotable.look_up("cllp_vs_mach_alpha",vmach,alppx);
	clldp=aerotable.look_up("clldp_vs_mach_alpha",vmach,alppx);

	//colling moment coefficient
    cll=cllap*alppx*alppx*s4phi+cllp*ppx*refl/(2.*dvbe)+clldp*dpx;
	...
	clmdq=aerotable.look_up("clmdq_vs_mach_alpha",vmach,alppx);

	//pitching moment coefficient
    double clmaref=clm0+clmp*s2phi+clmq*qqax*refl/(2.*dvbe)+clmdq*dqax;
    ...
    double clnr=clmq;
    double clndr=clmdq;
    double clnaref=clnp*s4phi+clnr*rrax*refl/(2.*dvbe)+clndr*drax;
...
void Missile::aerodynamics_der()
{
	...
	if(alppx<(alplimx-3.))
	{
		...
		double cma=DEG*(clm0p-clm0m)/(alpp-alpm)-cna*(xcgref-xcg)/refl;
