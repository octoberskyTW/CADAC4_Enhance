...
void number_objects(fstream &input,int &num_vehicles,int &num_missile,int &num_target)
{
	...
	//searching for # of missile and target objects
	...
	}while((num_missile+num_target)<num_vehicles);
...
//Arguments of object: module_list, num_modules, num_target to be passed to the constructor
...
//to be appended, limiting missile and target vehicles to 9 each (e.g.: SBEG1_t1)
				 ...
				 int nmissile,int ntarget)
{
	...
	Variable *data_target; //Variable array stored in 'Packet data' of type Target
	...
	//determining the total number of vector variables in com_missile6 and com_target3
	...
	int nvariables=ncom_missile6*nmissile+ncom_target3*ntarget+2*(p*nmissile+q*ntarget)
		-(nmissile-1);
...

{
	...
	//determining the number of missile and target packets
	for(i=0;i<num_vehicles;i++)
	{
		id=combus[i].get_id();
		loc=id.find("m");
...
void document_input(Document *doc_missile6,Document *doc_target3)
{
	...
	input.setf(ios::left);
	do{
		fcopy>>buffo;;//buffo gets MISSILE6, TARGET3, ENDTIME STOP and //comments
