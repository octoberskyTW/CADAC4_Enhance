				 ...
				 int nhyper)
{
	...
	int nvariables=ncom_hyper6*nhyper
		+2*(p*nhyper)-(nhyper-1);
	...
	for(i=0;i<num_vehicles;i++)
	{
		//determining wether 'Hyper' object
		id=combus[i].get_id();
		if(!id.find("h"))
		//'Hyper' object
		{
...
//030415 Adopted for HYPER simulation, PZi
//050214 Added third object for GSWS6, PZi
///////////////////////////////////////////////////////////////////////////////
...
void comscrn_data(Packet *combus,int num_vehicles)
{
	...
	//determining the number of hyperpackets
	for(i=0;i<num_vehicles;i++)
	{
		id=combus[i].get_id();
		loc=id.find("h");
		if(!loc) nhyper++;
		if(!combus[i].get_status() == 0)
		{
			time_index = i ;
		}
	}
	...
	Variable *data_c1=combus[time_index].get_data();
	double time=data_c1[0].real();
...
void document_input(Document *doc_hyper6)
{
	...
	input.setf(ios::left);
	do{
		fcopy>>buffo;;//buffo gets HYPER6,ENDTIME, STOP and //comments
		...
		if(!strcmp(buffo,"HYPER6")){
			...
			do{
				...
				else if(!strcmp(buffn,"AERO_DECK")||!strcmp(buffn,"PROP_DECK")||!strcmp(buffn,"WEATHER_DECK")){
					input<<"\t\t\t"<<buffn;
					fcopy.getline(line_clear,CHARL,'\n');
					input<<line_clear<<'\n';
				}
