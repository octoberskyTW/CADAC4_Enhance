...
void Hyper::def_ins()
{
	...
	hyper[301].init("frax_algnmnt",0,"Fractn to mod initial INS err state: XXO=XXO(1+frax)","ins","data","");
	...
	hyper[315].init("TBIC",0,0,0,0,0,0,0,0,0,"Comp T.M. of body wrt earth coor - None","ins","out","");
	...
	hyper[335].init("dbic",0,"INS computed vehicle distance from Earth center - m ","ins","out","");
	hyper[336].init("alphacx",0,"INS computed angle of attack - deg","ins","out","plot");
	hyper[337].init("betacx",0,"INS computed sideslip angle - deg","ins","out","plot");
	hyper[338].init("phibdcx",0,"INS computed geodetic Euler roll angle - deg","ins","out","plot");
	hyper[339].init("thtbdcx",0,"INS computed geodetic Euler pitch angle - deg","ins","out","plot");
	hyper[340].init("psibdcx",0,"INS computed geodetic Euler yaw angle - deg","ins","out","plot");
...
void Hyper::init_ins()
{
	...
	double frax_algnmnt=hyper[301].real();
...
void Hyper::ins(double int_step)
{
	...
	//selecting the Euler roll angle of flight mechanics (not for thtbdc=90 or =-90deg)
	if(mroll==0||mroll==1)
